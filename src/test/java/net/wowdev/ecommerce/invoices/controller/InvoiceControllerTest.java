...
class InvoiceControllerTest {
  ...
  @Test
  void gets() {
    when(service.findById(id)).thenReturn(TestFixtures.invoice(id));
    assertEquals(id, controller.get(id).getId());
  }

  @Test
  void lists() {
    when(service.findAll(any())).thenReturn(new PageImpl<>(java.util.List.of()));
    assertEquals(0, controller.list(0, 20).getTotalElements());
  }

  @Test
  void rejectsBadPage() {
    assertThrows(IllegalArgumentException.class, () -> controller.list(-1, 20));
    assertThrows(IllegalArgumentException.class, () -> controller.list(0, 101));
  }

  @Test
  void creates() {
    InvoiceDTO dto = TestFixtures.invoice(id);
  ...
  void mapsErrors() {
    assertEquals(
        ...
        controller.notFound(new InvoiceNotFoundException(id)).getStatusCode());
    assertEquals(
        HttpStatus.BAD_REQUEST,
        controller.badRequest(new IllegalArgumentException("bad")).getStatusCode());
  }
}
