...
class InvoiceProducerTest {
    ...
    private InvoiceCompleted completed;
    ...
    @BeforeEach void setUp() {
        ...
        completed = new InvoiceCompleted(UUID.randomUUID(), "tx", TestFixtures.order(), Instant.now(), "test");
    }
    ...
    @Test void publishesFailedImmediatelyWithoutTransaction() {
        var event = new InvoiceFailed(UUID.randomUUID(), "tx", TestFixtures.order(), Instant.now(), "bad", "test");
    ...
    @Test void publishesAfterTransactionCommit() throws NoSuchMethodException {
        var completedListener = InvoiceProducer.class
                .getMethod("publish", InvoiceCompleted.class)
                .getAnnotation(TransactionalEventListener.class);
        var failedListener = InvoiceProducer.class
                .getMethod("publish", InvoiceFailed.class)
                .getAnnotation(TransactionalEventListener.class);
